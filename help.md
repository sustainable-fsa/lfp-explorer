...
**What to show.** The *What to show* section at the top of the controls panel switches between the four bodies of data this explorer hosts, in the order the program reads them. **1 · Grazing periods** is *when* a drought has to happen for it to count at all — the window this whole program is measured inside. **2 · Drought monitor** is *where* it happened and how bad it got, week by week (see *The Drought Monitor* below). **3 · LFP eligibility** is the determination those two produce: which counties qualified in a program year, and for how many monthly payments (see *LFP eligibility* below). **4 · Disaster designations** is the wider paperwork around them — the Secretarial designations and Presidential declarations that the same drought triggers, and the other doors they open (see *Disaster designations* below). Each family brings its own controls, legend, and county card; what you have selected — the county, the year, the map position, the theme — comes along when you switch.
...
a designation nor contiguous to one, for the declaration type and disaster type
you have selected. Not eligible and not designated are answers, and the legend
says both in words.
...
**Drought, or everything.** The **Disaster type** control shows either drought alone — the LFP corner of this archive, and the default — or all twenty-two disaster types the portal reports, from hurricanes to insufficient chill hours. One consequence is worth knowing before it surprises you: **no Presidential declaration in this archive is coded for drought.** Not one of its 48,449 county rows. The Stafford Act is not the drought instrument, so *Presidential + Drought* is an empty map — an honest one, and the reason to switch the scope rather than the reason to doubt the map.
...
- **Two of the seventeen disaster "years" are not years.** One declaration is dated `0` (84 county rows) and one `2011, 2012` (10 rows). A slider cannot place either, so the map's earliest year is 2012 and the archive's 2011 designations are reachable only in the data table. Ask for 2011 and the map says so instead of pretending.
...
- **Amendments are numbered on the Presidential side only.** 452 of the 948 Presidential declarations here are amendments to an earlier one, and the card names the amendment number; Secretarial declarations carry their amendment history inside the description text instead, as the portal writes it.
