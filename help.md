...
**How these numbers reach FSA counties.** The archive is keyed to Census FIPS codes and this map always draws FSA service areas, so the designations arrive through the crosswalk described under *Two county keys* below. Where one FSA office administers several Census counties, the stronger role wins — Primary over Contiguous — and the card's *Combined from* row names each constituent county's own role, so a reader of a five-county office can see which part of it was named.
